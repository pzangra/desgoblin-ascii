...
from battle_system.enemy import generate_boss, generate_enemy, boss_list
...
class Game:
    ...
    def render_game_screen(self, game_map, extra_message: str = ""):
        """Render the current map, stats, and optional messages into the browser terminal."""
        output = game_map.render_map(self.hero)
        output += "\n\n"
        output += f"Player Stats: {self.hero.name} | HP: {self.hero.health}/{self.hero.health_max} | Gold: {self.hero.cashpile}\n"
        output += f"Level: {self.hero.level} | XP: {self.hero.experience}/{self.hero.experience_to_next_level}\n"
        output += f"Enemies Killed: {self.hero.enemies_killed}/{len(game_map.enemies) + self.hero.enemies_killed}\n"
        output += f"Bosses Defeated: {self.boss_defeated}/{self.total_bosses}\n"
        output += "Use W/A/S/D to move the player, 'I' to access inventory. Press Q to quit."
        if extra_message:
            output += "\n" + extra_message
